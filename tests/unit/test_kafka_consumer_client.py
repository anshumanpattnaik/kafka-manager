...
class TestKafkaConsumerClient:
    ...
    def test_kafka_consumer_client_start(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
        """ Test KafkaConsumerClient start method """
        assert kafka_consumer_client.start() is True
        assert kafka_consumer_client._running is True

    def test_kafka_consumer_client_start_already_running(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
    ...
    def test_kafka_consumer_client_start_with_kafka_error(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
        """ Test KafkaConsumerClient start method with Kafka Error """
        mock_kafka_consumer.side_effect = KafkaError('Kafka Error')
        assert kafka_consumer_client.start() is False
        assert kafka_consumer_client._running is False

    def test_kafka_consumer_client_consume(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
        """ Test consume method """
    ...
    def test_kafka_consumer_client_consume_with_no_message(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
    ...
    def test_kafka_consumer_client_consume_when_kafka_consumer_not_running(
        self, kafka_consumer_client: Mock
    ) -> None:
    ...
    def test_kafka_consumer_client_consume_with_kafka_error(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
    ...
    def test_kafka_consumer_client_consume_message_exits_kafka_consumer_not_running(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
        """ Test consume method when kafka consumer not running """
        ...
        mock_message_handler = MagicMock(
            side_effect=lambda v: setattr(kafka_consumer_client, '_running', False)
        )
    ...
    def test_kafka_consumer_client_stop(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
        ...
        kafka_consumer_client._consumer = mock_consume_instance
        assert kafka_consumer_client.stop() is True

        mock_consume_instance.unsubscribe.assert_called_once()

    def test_kafka_consumer_client_stop_with_kafka_error(
        self, kafka_consumer_client: Mock, mock_kafka_consumer: Mock
    ) -> None:
        ...
        assert kafka_consumer_client.stop() is False

    def test_kafka_consumer_client_is_running(
        self, kafka_consumer_client: Mock
    ) -> None:
        """ Test kafka consumer is running """
        kafka_consumer_client._running = True
        assert kafka_consumer_client.is_running() is True
